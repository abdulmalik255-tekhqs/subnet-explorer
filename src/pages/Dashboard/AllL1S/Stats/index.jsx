...
const Stats = ({ chain }) => {
  const overallUptime =
    validatorNodes.reduce((acc, v) => acc + parseFloat(v.uptime), 0) /
    validatorNodes.length;
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {validatorNodes.map((node, i) => (
                <tr
                  key={node.id}
                  className={`border-b border-gray-800/40 hover:bg-white/[0.02] transition-colors ${i === validatorNodes.length - 1 ? "border-0" : ""}`}
                >
                  <td className="px-5 py-2.5 text-blue-400 font-mono">
                    {node.id}
                  </td>
                  ...
                  <td className="px-5 py-2.5">
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${node.status === "Active" ? "bg-green-400" : "bg-yellow-400"}`}
                      />
                      {node.status}
                    </span>
                  </td>
