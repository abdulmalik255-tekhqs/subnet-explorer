...
const AccountERC = ({ address = "" }) => {
  const dispatch = useDispatch();
  const { transactionHeightAddress, AddressPages, ercTokentransaction } =
    useSelector((state) => state.transactions);
  const lastID = ercTokentransaction?.[ercTokentransaction.length - 1]?.number;
  ...
  useEffect(() => {
    ...
    dispatch.transactions.getTransactionByAddressERCWithPagination({
  ...
  }, [address, limit, page, transactionHeightAddress]);
    ...
    <div className="bg-white  border border-bryt-grey-200 rounded-[12px] shadow-txBox mt-3">
      <div className="flex items-center justify-between py-5 px-[17px]">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[14.5px]">
            Latest {ercTokentransaction?.length} from a total of{" "}
        ...
        <div className="flex items-center gap-2">
          <div
            onClick={() =>
              downloadAsCSV(
                ercTokentransaction,
                `account_erc_token_transactions_${page}.csv`
              )
            }
      ...
      <div className="w-full overflow-auto">
        <table className="w-full border-collapse">
          ...
          <tbody>
            {ercTokentransaction?.length > 0 ? (
              ercTokentransaction?.map((erc) => (
                <tr
                  key={erc?.id}
